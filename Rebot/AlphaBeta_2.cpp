...
/*
bool creatmove(int depth, int alpha, int beta){
...
bool makemove(int depth){//将节点添加到movelist然后添加到棋盘
	if (flag[depth] > possiblecount[depth])
		return false;
	moveList[MoveNum].x = possiblemove[depth - 1][possiblecount[depth - 1]][0].x;// possiblemove[depth][flag[depth]][0].x;
	moveList[MoveNum].y = possiblemove[depth - 1][possiblecount[depth - 1]][0].y;// possiblemove[depth][flag[depth]++][0].y;
	MoveNum++;
	moveList[MoveNum].x = possiblemove[depth - 1][possiblecount[depth - 1]][1].x;// possiblemove[depth][flag[depth]][1].x;
	moveList[MoveNum].y = possiblemove[depth - 1][possiblecount[depth - 1]][1].y;// possiblemove[depth][flag[depth]++][1].y;
	MoveNum++;
	if (depth % 2 == 1)
	{
...
int alphaBeta( int depth, long alpha, long beta)
{
	...
	//剪枝：creatmove,makemove，value,unmove
	{
	int i, j, m, n;
	int wmax = 0, wmin = 18, hmax = 0, hmin = 18;
	for (i = 0; i < 19; i++)
		...
		for (; n <=hmax; n++,m=wmin)
		{
			...
			for (; m <=wmax; m++)
			{
				...
				if (board[i][j] == NOSTONE){
					...
					moveList[MoveNum].x = i;
					moveList[MoveNum].y = j;
					MoveNum++;
					count++;
				}
				else
				...
				if (board[m][n] == NOSTONE){
					...
					moveList[MoveNum].x = m;
					moveList[MoveNum].y = n;
					MoveNum++;
					count++;
				}
				else
