...
int enemycountnum(){
	...
	while (flag)
	{
		if (c>0)
		{
			if ((area[8] != area[8 + c]) && ( area[8 + c]!=NOSTONE))
			{
				enemycountall++;
				c++;
			}
			else
			if ((area[8] != area[8 + c])&&(area[8] + c == NOSTONE) && (breaknum > 0) && (area[8 + c - 1] == area[8 + c + 1]))
			{
				c++;
				c++;
				breaknum--;
...
int countnum(){
	...
	while (flag)
	{
		if (c>0)
		{
			...
			if ((area[8] + c == NOSTONE) &&(breaknum > 0) && (area[8] == area[8 + c + 1]))
				{
					c++;
					c++;
					breaknum--;
...
//改进
	for (int i = 0; i < 8; i++)
		{
			analysis[i] = area[8-left -1 + i];
		}
	if ((analysis[0] == enemy) && (analysis[6] == enemy))
		//evaluation.BLANK;//死五
	{
		if (mytype == TYPE){
			status.road[0]++;
	...
	else if ((analysis[0] != enemy) && (analysis[6] != enemy))
			{//evaluation.LIVE_FIVE;//活五
		if (mytype == TYPE){
			status.road[10]++;
			status.road[9]--;
...
void checkfour(){
	...
	int count = countall;
	for (int i = 0; i < 8; i++){
		analysis[i] = area[8- left - 2 + i];
	}
//判断4
	if ((analysis[1] == enemy) && (analysis[6] == enemy))
		evaluation.BLANK;//死四
	else if ((analysis[1] == enemy) && (analysis[6] != enemy) && (analysis[7] != enemy) || (analysis[6] == enemy) && (analysis[1] != enemy) && (analysis[0] != enemy))
		evaluation.SLEEP_FOUR;
	else if ((analysis[0] != enemy) && (analysis[7] != enemy) && (analysis[1] != enemy) && (analysis[6] != enemy))
